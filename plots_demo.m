...
DEP_carrier = 1 * 10*log10(abs(fftshift(fft(cos(0 * t), res * 1023))));
DEP_message = 10*log10(abs(fftshift(fft(message_samples, res * 1023))));
DEP_SV = 10*log10(abs(fftshift(fft(srx, res * 1023))));
...
grid on
axis tight
plot(f, DEP_carrier - max(DEP_carrier));
plot(f, DEP_message - max(DEP_carrier));
plot(f, DEP_SV - max(DEP_carrier))
legend('DEP\_carrier', 'DEP\_nav\_message', 'received signal')
xlabel('Hz')
